...
// === CUSTOM OBFUSCATION FOR GEARSWAPPER ===

tasks.register("obfuscateGearSwapper") {
    dependsOn(gearSwapperJar)
    group = "build"
    description = "Obfuscates GearSwapper classes and repacks the JAR"

    val gearSwapperJarTask = gearSwapperJar.get()
    val inputJar = gearSwapperJarTask.archiveFile.get().asFile
    val outputJar = layout.buildDirectory.file("libs/gearswapper-${gearSwapperJarTask.archiveVersion.get()}-obfuscated.jar")
    val tempDir = layout.buildDirectory.dir("temp-obf-gearswapper")
    ...
    doLast {
        ...
        // Extract JAR
        ZipFile(inputJar).use { zip: ZipFile ->
            val entries = zip.entries()
            while (entries.hasMoreElements()) {
                val entry = entries.nextElement()
                val outFile = File(tempDirFile, entry.name)
        ...
        // Classes that MUST stay unobfuscated for RuneLite discovery
        val protectedFiles = setOf(
            "runelite-plugin.properties",
            "com/tonic/plugins/gearswapper/GearSwapperPlugin.class",
            "com/tonic/plugins/gearswapper/GearSwapperConfig.class"
        )

        // Packages to protect completely (UI needs reflection)
        val protectedPackages = listOf(
            "com/tonic/plugins/gearswapper/ui/",
            "com/tonic/plugins/gearswapper/sdn/",
            "com/tonic/plugins/gearswapper/friendshare/"
        )

        var counter = 1
        println("[Obfuscation] Starting obfuscation process (gearswapper)...")
        var obfuscatedCount = 0
        var protectedCount = 0

        fileTree(tempDirFile).matching { include("**/*.class") }.forEach { file ->
            val relativePath = file.toRelativeString(tempDirFile).replace("\\", "/")
            
            // Check if should be protected
            val shouldProtect = relativePath in protectedFiles ||
                protectedPackages.any { relativePath.startsWith(it) }

            if (shouldProtect) {
                protectedCount++
                println("[Obfuscation] ✅ PROTECTED: $relativePath")
            } else {
                // OBFUSCATE: Rename and XOR encrypt
                val newName = "a${counter++}.class"
                val newFile = File(file.parentFile, newName)
                file.renameTo(newFile)

                // XOR encrypt with key 0x55
                val bytes = newFile.readBytes()
                val encrypted = bytes.map { byte: Byte -> (byte.toInt() xor 0x55).toByte() }.toByteArray()
                newFile.writeBytes(encrypted)
                obfuscatedCount++
                println("[Obfuscation] 🔐 ENCRYPTED: $relativePath -> $newName")
            }
        }

        println("[Obfuscation] Complete! Protected: $protectedCount, Obfuscated: $obfuscatedCount")

        // Repack JAR
        ZipOutputStream(outputJarFile.outputStream()).use { zos: ZipOutputStream ->
            tempDirFile.walkTopDown().filter { it.isFile }.forEach { file ->
                val entryName = file.toRelativeString(tempDirFile).replace("\\", "/")
        ...
        println("[Obfuscation] Output: ${outputJarFile.absolutePath}")
        tempDirFile.deleteRecursively()
    }
}

// Obfuscated Gear Swapper JAR using ProGuard
val obfuscatedGearSwapperJar by tasks.registering(proguard.gradle.ProGuardTask::class) {
    dependsOn(gearSwapperJar)
    
    val inputJar = gearSwapperJar.get().archiveFile.get().asFile
    val outputJar = file("build/libs/gearswapper-1.5.52-obfuscated.jar")
    
    injars(inputJar)
    outjars(outputJar)
    
    // Add RuneLite client and all dependencies to libraryjar (deduplicated)
    val allLibs = (configurations.compileClasspath.get().files + configurations.runtimeClasspath.get().files).toSet()
    allLibs.forEach { libraryjars(it) }
    ...
    printmapping(file("build/proguard-mapping.txt"))
}
