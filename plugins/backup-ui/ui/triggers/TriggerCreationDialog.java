...
public class TriggerCreationDialog extends JDialog {
    ...
    // GFX config (Dedicated fields to prevent conflict)
    ...
    private void createConfigPanels() {
        ...
        // GFX config panel (separate from animation with proper labels)
    ...
    private JPanel createGfxConfigPanel() {
        ...
        panel.add(gfxIdField, gbc); // Use dedicated field
        ...
        gbc.weightx = 1.0;
        gbc.insets = new Insets(5, 0, 5, 0);
        panel.add(gfxTargetFilterComboBox, gbc); // Use dedicated field
        ...
        gbc.weightx = 1.0;
        gbc.insets = new Insets(5, 0, 5, 0);
        panel.add(gfxCooldownField, gbc); // Use dedicated field
        ...
        try {
            ...
            if (type == TriggerType.ANIMATION) {
                ...
                try {
                    ...
                    config.setCooldownMs(Long.parseLong(gfxCooldownField.getText().trim()));
                    config.setTargetFilterByValue((String) gfxTargetFilterComboBox.getSelectedItem());
                } catch (NumberFormatException e) {
                    JOptionPane.showMessageDialog(this, "Invalid GFX ID or cooldown", "Error",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
    ...
    private void testTrigger() {
        ...
        try {
            ...
            if (type == TriggerType.ANIMATION) {
                ...
                config.setGfxId(Integer.parseInt(animationIdField.getText().trim()));
                config.setCooldownMs(0); // No cooldown for testing
                config.setTargetFilterByValue((String) animationTargetFilterComboBox.getSelectedItem());
            }
