...
public class TriggerEngine {
    ...
    private void initializeHandlers() {
        ...
        // Register Player Spawned trigger handler
        handlers.add(new PlayerSpawnedTriggerHandler(this));
            ...
            for (int i = 0; i < actions.size(); i++) {
                ...
                try {
                    ...
                    if (action.isEnabled()) {
                        ...
                        logger.info("[Trigger Engine] Action {} executed successfully",
    ...
    private void loadTriggersFromConfig() {
        try {
            ...
            if (triggersJson != null && !triggersJson.trim().isEmpty()) {
                ...
                logger.info("[Trigger Engine] Loaded {} triggers from configuration (skipped test triggers)",
                        validTriggers.size());
    ...
    private void saveTriggerToConfigAsync(Trigger trigger) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                logger.error("[Trigger Engine] Error in async save for trigger {}: {}", trigger.getName(),
                        e.getMessage(), e);
            }
    ...
    public void saveTriggersToConfig() {
        try {
            ...
            if (triggersJson != null) {
                configManager.setConfiguration(GearSwapperConfig.GROUP, "triggers", triggersJson);
                logger.info("[Trigger Engine] Saved {} triggers to configuration (excluded test triggers)",
                        allTriggers.size());
            }
        } catch (Exception e) {
    ...
    public void createSampleTriggers() {
        try {
            ...
            Trigger dragonFireTrigger = new Trigger("dragon_fire_sample", "Dragon Fire Response",
    ...
    public void onPlayerSpawned(PlayerSpawned event) {
        if (!enabled.get() || event.getPlayer() == null) {
            return;
        }

        TriggerEvent triggerEvent = new TriggerEvent(TriggerEventType.PLAYER_SPAWNED, event);
        processEvent(triggerEvent);
    }
}
