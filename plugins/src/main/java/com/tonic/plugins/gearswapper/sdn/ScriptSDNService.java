...
public class ScriptSDNService {
     ...
     */
    private Request.Builder addAuthHeaders(Request.Builder builder, NoidUser user) {
        if (user == null) {
            return builder;
        }
        return builder
     ...
     */
    public CompletableFuture<List<ScriptEntry>> fetchScripts(NoidUser user) {
     ...
     * Submit a new script
     */
    public CompletableFuture<ScriptEntry> submitScript(NoidUser user, String name, String description, String content) {
        return CompletableFuture.supplyAsync(() -> {
            try {
     ...
     */
    public CompletableFuture<ScriptEntry> updateScript(NoidUser user, String scriptId, String name, String description,
            String content) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                JsonObject body = new JsonObject();
     ...
     */
    public CompletableFuture<Boolean> deleteScript(NoidUser user, String scriptId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                Request request = addAuthHeaders(new Request.Builder(), user)
                        .url(API_URL + "/" + scriptId)
     ...
     */
    public CompletableFuture<VoteResult> voteScript(NoidUser user, String scriptId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                Request request = addAuthHeaders(new Request.Builder(), user)
