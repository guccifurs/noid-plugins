...
public class TriggerEditDialog extends JDialog
{
    ...
    private void loadTriggerData()
    {
        ...
        if (!originalTrigger.getActions().isEmpty())
        {
            ...
            {
                System.out.println("[DEBUG] First action is not a GearSwapAction: " + firstAction.getClass().getSimpleName());
            }
        }
        else
        {
        ...
        try
        {
            ...
            else if (type == TriggerType.HP)
            {
                try
                {
                    ...
                    if (!hpText.matches("\\d+"))
                    {
                        JOptionPane.showMessageDialog(this, "HP threshold must be a whole number (e.g., 50, not 50.0)", "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
            ...
            JOptionPane.showMessageDialog(this, "Trigger updated successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
            dispose();
            
        }
        catch (Exception e)
        {
            JOptionPane.showMessageDialog(this, "Error updating trigger: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
