...
@Slf4j
@PluginDescriptor(
    name = "Gear Swapper 3.0",
    description = "Gear Swapper 3.0 - advanced PvP loadouts with trigger engine v3 and debug system",
    tags = {"gear", "swap", "pvp", "pk", "loadout", "triggers", "debug"}
)
public class GearSwapperPlugin extends Plugin
{
    ...
    protected void startUp() throws Exception
    {
        try
        {
            ...
            Logger.norm("[Gear Swapper] Gear Swapper v3.32 - Animation trigger Local/Target filter added");
            
            // Test different logging methods
            System.out.println("=== GEAR SWAPPER PLUGIN: STARTUP TEST ===");
            ...
            if (configManager != null)
            {
                try
                {
                    ...
                    Boolean savedLooperEnabled = configManager.getConfiguration(GearSwapperConfig.GROUP, "looperEnabled", Boolean.class);
                ...
                try
                {
                    Boolean savedHeatmapEnabled = configManager.getConfiguration(GearSwapperConfig.GROUP, "clickHeatmapEnabled", Boolean.class);
                    clickHeatmapEnabled = savedHeatmapEnabled != null && savedHeatmapEnabled;

                    Integer savedMinDelay = configManager.getConfiguration(GearSwapperConfig.GROUP, "minClickDelay", Integer.class);
                    if (savedMinDelay != null && savedMinDelay >= 0 && savedMinDelay <= 600)
                    {
                        minClickDelay = savedMinDelay;
                    }

                    Integer savedMaxDelay = configManager.getConfiguration(GearSwapperConfig.GROUP, "maxClickDelay", Integer.class);
                    if (savedMaxDelay != null && savedMaxDelay >= 0 && savedMaxDelay <= 600)
                    {
                        maxClickDelay = savedMaxDelay;
                    }

                    Boolean savedForceOneTick = configManager.getConfiguration(GearSwapperConfig.GROUP, "forceOneTickEnabled", Boolean.class);
                    forceOneTickEnabled = savedForceOneTick != null && savedForceOneTick;

                    Integer savedForceThreshold = configManager.getConfiguration(GearSwapperConfig.GROUP, "forceTickThresholdMs", Integer.class);
    ...
    public void onMenuOptionClicked(MenuOptionClicked event)
    {
        try
        {
            ...
            // === Left Click Cast: intercept player Attack/Fight clicks ===
            if (config != null && config.leftClickCastEnabled())
            {
                if (handleLeftClickCast(event, option, target))
                {
                    // Successfully handled via spell cast - suppress original menu action
                    event.consume();
                    return;
                }
            }
            ...
            {
                Logger.norm("[Gear Swapper] Detected brew click: itemId=" + itemId + ", option=" + option + ", target=" + target + ", action=" + event.getMenuAction());
            ...
            {
                Logger.norm("[Gear Swapper] Detected Sanfew click: itemId=" + itemId + ", option=" + option + ", target=" + target + ", action=" + event.getMenuAction());
                    ...
                    public void keyPressed(KeyEvent e)
                    {
                        ...
                        if (loadoutKeybind != null && loadoutKeybind.matches(e))
                        {
                            ...
                            if (panel != null)
                            {
                                try
                                {
                                    Method getLoadoutDataMethod = panel.getClass().getMethod("getLoadoutData", int.class);
                                    ...
                                    if (loadoutData != null)
                                    {
                                        ...
                                        Logger.norm("[Gear Swapper DEBUG] Got live data: name='" + currentName + "', items='" + currentItems + "', attack=" + currentAttack);
                                    }
                                    else
                                    {
                                        Logger.norm("[Gear Swapper DEBUG] LoadoutData is null for loadout " + loadoutNum);
                                    }
                                }
                                catch (Exception ex)
                                {
                                    Logger.norm("[Gear Swapper DEBUG] Exception getting panel data: " + ex.getMessage());
                                }
                            }
                            else
                            {
    ...
    public void setForceTickThresholdMs(int thresholdMs)
    {
        ...
        if (configManager != null)
        {
            try
            {
                configManager.setConfiguration(GearSwapperConfig.GROUP, "forceTickThresholdMs", this.forceTickThresholdMs);
            }
            catch (Exception e)
            {
    ...
    public void onConfigChanged(ConfigChanged event)
    {
        ...
        // No longer need to handle unlockedLoadoutsCount changes since we have unlimited loadouts
        // Check if freeze overlay setting was changed
        else if ("showFreezeOverlay".equals(event.getKey()))
        {
            boolean enabled = Boolean.parseBoolean(event.getNewValue());
            Logger.norm("[Gear Swapper] Freeze overlay " + (enabled ? "enabled" : "disabled"));
        ...
        else if ("showDebugOverlay".equals(event.getKey()))
        {
            boolean enabled = Boolean.parseBoolean(event.getNewValue());
        ...
        else if ("enableMouseCircleTest".equals(event.getKey()))
        {
            boolean enabled = Boolean.parseBoolean(event.getNewValue());
                ...
                for (int i = 0; i < items.length; i++)
                {
                    ...
                    if (item != null && item.getId() > 0)
                    {
                        ...
                        if (itemName != null && !itemName.isEmpty())
                        {
                            ...
                            Logger.norm("[Gear Swapper] Found equipped item: " + itemName + " (ID: " + item.getId() + ", Qty: " + item.getQuantity() + ")");
                        }
                    }
                }
            }
    ...
    public void copyCurrentGearToLoadout(int loadoutNum)
    {
        ...
        Static.invoke(() -> {
            try
            {
                ...
                Logger.norm("[Gear Swapper] Got gear commands: " + (gearCommands.isEmpty() ? "(empty)" : gearCommands.substring(0, Math.min(100, gearCommands.length())) + "..."));
                ...
                if (panel != null)
                {
                    ...
                    if (data != null)
                    {
                        ...
                        Logger.norm("[Gear Swapper] Copied current gear to loadout " + loadoutNum + " (was: " + (oldItems.isEmpty() ? "(empty)" : oldItems.length() + " chars") + ", now: " + gearCommands.length() + " chars)");
                    }
                    else
                    {
            ...
            if (panel != null)
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    try
                    {
                        ...
                        {
                            Logger.norm("[Gear Swapper Trigger] Found matching loadout '" + gearSetName + "' at slot " + i);
            ...
            if (lower.startsWith("tick:"))
            {
                ...
                if (currentBatch.length() > 0)
                {
                    ...
                    if (isFirstBatch)
                    {
                        ...
                        Logger.norm("[Gear Swapper] Executing first batch immediately: " + batchCommands.replace("\n", "; "));
                        Static.invoke(() -> {
                    ...
                    else
                    {
                        ...
                        Logger.norm("[Gear Swapper] Scheduled batch for tick " + targetTick + ": " + batchCommands.replace("\n", "; "));
                    }
    ...
    private void scheduleScriptWithMeleeRange(String executeStr, boolean attackFlag)
    {
        ...
        if (localPos != null && targetPos != null)
        {
            ...
            if (dist <= 3)
            {
                ...
                {
                    // Already in melee range: now schedule the rest of the script using Tick: semantics
        ...
        // Not yet in range: start walking towards target (3 tiles) and gate for up to 2 ticks
        ...
        if (trimmed.contains(":"))
        {
            ...
            if (parts.length >= 2)
            {
                ...
                else if (command.equalsIgnoreCase("Tick"))
                {
                    ...
                    Logger.norm("[Gear Swapper] Tick:" + value + " encountered in direct execution (no-op here, handled by scheduler)");
                }
     ...
     *  - "self_frozen" / "player_frozen" / "me_frozen" / variants -> playerFreezeTicks > 0
     */
    ...
    private boolean evalAtomicCondition(String lowerCond, String originalCond)
    {
        ...
        for (String op : operators)
        {
            ...
            if (idx > 0)
            {
                ...
                try
                {
                    ...
                    Logger.norm("[Gear Swapper] Condition: " + left + " " + op + " " + right + " => " + leftValue + " " + op + " " + right + " = " + result);
                    return result;
                }
                catch (NumberFormatException e)
                {
        ...
        if (lowerCond.equals("target_frozen") || lowerCond.equals("target frozen") || lowerCond.equals("targetfrozen"))
        {
            return targetFreezeTicks > 0;
        }

        if (lowerCond.equals("has_target") || lowerCond.equals("target_exists") || lowerCond.equals("has target") || lowerCond.equals("hastarget"))
        {
    ...
    /**
     * Check if the currently equipped weapon matches any of the Left Click Cast weapon patterns.
     */
    private boolean isLeftClickCastWeaponEquipped()
    {
        if (config == null)
        {
            return false;
        ...
        String patterns = config.leftClickCastWeapons();
        if (patterns == null)
        {
            return false;
        }
        ...
        for (String raw : lines)
        {
            ...
            if (matchesItemPattern(weaponName, pattern))
            {
                return true;
            }
        }
    ...
    /**
     * Handle Left Click Cast logic for a given menu click.
     */
    private boolean handleLeftClickCast(MenuOptionClicked event, String option, String target)
    {
        if (client == null || client.getLocalPlayer() == null)
        {
            return false;
        ...
        // Only consider player Attack/Fight options
        ...
        String optLower = option.toLowerCase();
        if (!(optLower.contains("attack") || optLower.contains("fight")))
        {
            return false;
        }

        if (!isLeftClickCastWeaponEquipped())
        {
            return false;
        }

        String spellName = config.leftClickCastSpell();
        if (spellName == null || spellName.trim().isEmpty())
        {
            return false;
        }

        int playerIndex = event.getId();
        if (playerIndex < 0)
        {
            return false;
        }

        // Resolve the Player using PlayerQuery by matching the PlayerEx id (backed by index)
        PlayerEx targetEx = new PlayerQuery()
            .keepIf(px -> px != null && px.getId() == playerIndex)
            .first();

        if (targetEx == null || targetEx.getPlayer() == null)
        {
            return false;
        }

        return castLeftClickSpellOnTarget(spellName, targetEx.getPlayer());
    }

    /**
     * Cast the configured Left Click Cast spell on the given player using Ancient spell mapping.
     */
    private boolean castLeftClickSpellOnTarget(String spellName, Player target)
    {
        try
        {
            if (target == null)
            ...
            String spellLower = spellName.trim().toLowerCase();
            if (spellLower.isEmpty())
            {
                return false;
            }
            ...
            {
                effectiveMagic = 99;
                Logger.norm("[Gear Swapper] LCC: Sanfew flag active - boosted=" + boostedMagic + ", effective=" + effectiveMagic);
            }
            else if (brewClickedThisTick)
            {
                effectiveMagic = Math.max(1, boostedMagic - 11);
                Logger.norm("[Gear Swapper] LCC: Brew flag active - boosted=" + boostedMagic + ", effective=" + effectiveMagic);
            }

            if (!canCastSpell(spellLower, effectiveMagic))
            {
                Logger.norm("[Gear Swapper] LCC: Cannot cast '" + spellName + "' at effective level " + effectiveMagic + " (boosted " + boostedMagic + "), skipping.");
                return false;
            }

            Ancient spell = getAncientSpell(spellLower);
            if (spell == null)
            {
                Logger.norm("[Gear Swapper] LCC: Unknown spell '" + spellName + "', skipping.");
                return false;
            }

            if (!spell.canCast())
            {
                Logger.norm("[Gear Swapper] LCC: Spell cannot be cast for '" + spellName + "' (runes/gear/book), skipping.");
                return false;
            }
            ...
            Static.invoke(() -> {
                ...
                Logger.norm("[Gear Swapper] Left Click Cast: Cast '" + spellName + "' on target '" + target.getName() + "'");
            });
        ...
        {
            Logger.error("[Gear Swapper] Error casting Left Click Cast spell '" + spellName + "': " + e.getMessage());
            return false;
        }
    }
                    ...
                    for (String actionName : new String[]{"Wear", "Equip", "Wield"})
                    {
                        for (String action : actions)
                        {
                            if (action != null && action.equalsIgnoreCase(actionName))
                            {
                                ...
                                Logger.norm("[Gear Swapper] Equipped: " + item.getName() + (clickHeatmapEnabled ? " (with heatmap delay)" : ""));
    ...
    private boolean castSpellWithReturn(String spellName)
    {
        try
        {
            ...
            Static.invoke(() -> {
                ...
                {
                    effectiveMagic = 99;
                    Logger.norm("[Gear Swapper] Sanfew flag active this tick - boosted=" + boostedMagic + ", effective=" + effectiveMagic);
                }
                else if (brewClickedThisTick)
                {
                    effectiveMagic = Math.max(1, boostedMagic - 11);
                    Logger.norm("[Gear Swapper] Brew flag active this tick - boosted=" + boostedMagic + ", effective=" + effectiveMagic);
                }

                Logger.norm("[Gear Swapper] Attempting to cast '" + spellName + "' with effective magic " + effectiveMagic);

                if (!canCastSpell(lower, effectiveMagic))
                {
                    Logger.norm("[Gear Swapper] Cannot cast '" + spellName + "' at effective level " + effectiveMagic + " (boosted " + boostedMagic + "), skipping.");
                    return;
                }
                ...
                if (!spell.canCast())
                {
                    Logger.norm("[Gear Swapper] Spell cannot be cast for '" + spellName + "' (runes/gear/book), skipping.");
                    return;
                }
    ...
    private void castSpell(String spellName)
    {
        try
        {
            ...
            Runnable castTask = () -> {
                ...
                Logger.norm("[Gear Swapper] Cast spell: " + spellName + (clickHeatmapEnabled ? " (with heatmap delay)" : ""));
            };
        ...
        try
        {
            ...
            Static.invoke(() -> {
                ...
                for (String prayerName : prayerOptions)
                {
                    ...
                    // bypass the PrayerAPI.hasLevelFor gating and click the widget directly using VitaLite's WidgetAPI.
        ...
        try
        {
            Static.invoke(() -> {
                ...
                try {
                    ...
                    if (specEnergy < 50) {
                        Logger.norm("[Gear Swapper] Cannot enable special attack - not enough energy: " + specEnergy + "%");
                        return;
                    }
    ...
    private PrayerAPI getPrayerByName(String name)
    {
        switch (name)
        {
            case "thick skin": return PrayerAPI.THICK_SKIN;
            case "burst of strength": return PrayerAPI.BURST_OF_STRENGTH;
            case "clarity of thought": return PrayerAPI.CLARITY_OF_THOUGHT;
            case "sharp eye": return PrayerAPI.SHARP_EYE;
            case "mystic will": return PrayerAPI.MYSTIC_WILL;
            case "rock skin": return PrayerAPI.ROCK_SKIN;
            case "superhuman strength": return PrayerAPI.SUPERHUMAN_STRENGTH;
            case "improved reflexes": return PrayerAPI.IMPROVED_REFLEXES;
            case "rapid restore": return PrayerAPI.RAPID_RESTORE;
            case "rapid heal": return PrayerAPI.RAPID_HEAL;
            case "protect item": return PrayerAPI.PROTECT_ITEM;
            case "hawk eye": return PrayerAPI.HAWK_EYE;
            case "mystic lore": return PrayerAPI.MYSTIC_LORE;
            case "steel skin": return PrayerAPI.STEEL_SKIN;
            case "ultimate strength": return PrayerAPI.ULTIMATE_STRENGTH;
            case "incredible reflexes": return PrayerAPI.INCREDIBLE_REFLEXES;
            case "protect from magic": return PrayerAPI.PROTECT_FROM_MAGIC;
            case "protect from missiles": return PrayerAPI.PROTECT_FROM_MISSILES;
            case "protect from melee": return PrayerAPI.PROTECT_FROM_MELEE;
            case "eagle eye": return PrayerAPI.EAGLE_EYE;
            case "mystic might": return PrayerAPI.MYSTIC_MIGHT;
            case "retribution": return PrayerAPI.RETRIBUTION;
            case "redemption": return PrayerAPI.REDEMPTION;
            case "smite": return PrayerAPI.SMITE;
            case "preserve": return PrayerAPI.PRESERVE;
            case "chivalry": return PrayerAPI.CHIVALRY;
            case "piety": return PrayerAPI.PIETY;
            case "rigour": return PrayerAPI.RIGOUR;
            case "augury": return PrayerAPI.AUGURY;
            default: return null;
        }
    }
    ...
    private boolean matchesItemPattern(String itemName, String pattern)
    {
        ...
        if (lowerItemName.equals(lowerPattern))
        {
            return true;
        }
        ...
        if (lowerPattern.endsWith("*"))
        {
            ...
            return lowerItemName.contains(prefix);
        }
            ...
            Static.invoke(() -> {
                ...
                for (String spellName : spellOptions)
                {
                    ...
                    if (target != null)
                    {
                        PlayerEx targetEx = new PlayerEx(target);
                        spell.castOn(targetEx);
                        Logger.norm("[Gear Swapper] Cast on target '" + target.getName() + "' using spell: " + spellName);
                    }
                    else
                    {
                        MagicAPI.cast(spell);
    ...
    private Ancient getAncientSpell(String spellName)
    {
        switch (spellName)
        {
            case "ice barrage": return Ancient.ICE_BARRAGE;
            case "ice blitz": return Ancient.ICE_BLITZ;
            case "ice burst": return Ancient.ICE_BURST;
            case "ice rush": return Ancient.ICE_RUSH;
            case "blood barrage": return Ancient.BLOOD_BARRAGE;
            case "blood blitz": return Ancient.BLOOD_BLITZ;
            case "blood burst": return Ancient.BLOOD_BURST;
            case "blood rush": return Ancient.BLOOD_RUSH;
            case "shadow barrage": return Ancient.SHADOW_BARRAGE;
            case "shadow blitz": return Ancient.SHADOW_BLITZ;
            case "shadow burst": return Ancient.SHADOW_BURST;
            case "shadow rush": return Ancient.SHADOW_RUSH;
            case "smoke barrage": return Ancient.SMOKE_BARRAGE;
            case "smoke blitz": return Ancient.SMOKE_BLITZ;
            case "smoke burst": return Ancient.SMOKE_BURST;
            case "smoke rush": return Ancient.SMOKE_RUSH;
            default: return null;
        }
    }
    ...
    private boolean canCastSpell(String spellName, int magicLevel)
    {
        switch (spellName)
        {
            case "ice barrage": return magicLevel >= 94;
            case "blood barrage": return magicLevel >= 92;
            case "shadow barrage": return magicLevel >= 88;
            case "smoke barrage": return magicLevel >= 86;
            case "ice blitz": return magicLevel >= 82;
            case "blood blitz": return magicLevel >= 80;
            case "shadow blitz": return magicLevel >= 76;
            case "smoke blitz": return magicLevel >= 74;
            case "ice burst": return magicLevel >= 70;
            case "blood burst": return magicLevel >= 68;
            case "shadow burst": return magicLevel >= 64;
            case "smoke burst": return magicLevel >= 62;
            case "ice rush": return magicLevel >= 58;
            case "blood rush": return magicLevel >= 56;
            case "shadow rush": return magicLevel >= 52;
            case "smoke rush": return magicLevel >= 50;
    ...
    public void onGameTick(GameTick event)
    {
        ...
        if (client != null)
        {
            if (playerFreezeTicks > 0 && client.getLocalPlayer() != null)
            {
                ...
                if (currentPlayerLoc != null && playerFreezeLocation != null && !currentPlayerLoc.equals(playerFreezeLocation))
                {
                    playerFreezeTicks = 0;
                    playerFreezeImmunityTicks = 0;
                    playerFreezeLocation = null;
            ...
            if (targetFreezeTicks > 0 && currentTarget != null)
            {
                ...
                if (currentTargetLoc != null && targetFreezeLocation != null && !currentTargetLoc.equals(targetFreezeLocation))
                {
                    targetFreezeTicks = 0;
                    targetFreezeImmunityTicks = 0;
                    targetFreezeLocation = null;
        ...
        if (!scheduledScriptTasks.isEmpty())
        {
            ...
            for (ScriptTask task : scheduledScriptTasks)
            {
                ...
                else if (task.targetTick < tickCounter)
                {
                    ...
                    Logger.warn("[Gear Swapper] Running late scheduled task: " + task.description + " (was for tick " + task.targetTick + ", now " + tickCounter + ")");
                    try
                    {
                        task.action.run();
                    }
        ...
        {
            executeCommands(looperScript, false);
        }
                ...
                else
                {
                    ...
                    if (candidate.equals(playerPos))
                    {
                        ...
                        if (distance > desired + 0.1)
                        {
                            ...
                            candidate = new WorldPoint(playerPos.getX() + stepX, playerPos.getY() + stepY, targetPos.getPlane());
                        }
    ...
    private void scheduleNextHeatmapCommand()
    {
        ...
        {
            // If Force 1 tick is enabled, clamp delay so execution stays within the threshold window of the current tick.
    ...
    private int getSkillLevel(String skillName)
    {
        ...
        try
        {
            switch (lower)
            {
                case "attack": return SkillAPI.getBoostedLevel(Skill.ATTACK);
                case "strength": return SkillAPI.getBoostedLevel(Skill.STRENGTH);
                case "defence": case "defense": return SkillAPI.getBoostedLevel(Skill.DEFENCE);
                case "ranged": case "range": return SkillAPI.getBoostedLevel(Skill.RANGED);
                case "prayer": return SkillAPI.getBoostedLevel(Skill.PRAYER);
                case "magic": case "mage": return SkillAPI.getBoostedLevel(Skill.MAGIC);
                case "hitpoints": case "hp": case "health": return SkillAPI.getBoostedLevel(Skill.HITPOINTS);
                case "agility": return SkillAPI.getBoostedLevel(Skill.AGILITY);
                case "herblore": return SkillAPI.getBoostedLevel(Skill.HERBLORE);
                case "thieving": return SkillAPI.getBoostedLevel(Skill.THIEVING);
                case "crafting": return SkillAPI.getBoostedLevel(Skill.CRAFTING);
                case "fletching": return SkillAPI.getBoostedLevel(Skill.FLETCHING);
                case "slayer": return SkillAPI.getBoostedLevel(Skill.SLAYER);
                case "hunter": return SkillAPI.getBoostedLevel(Skill.HUNTER);
                case "mining": return SkillAPI.getBoostedLevel(Skill.MINING);
                case "smithing": return SkillAPI.getBoostedLevel(Skill.SMITHING);
                case "fishing": return SkillAPI.getBoostedLevel(Skill.FISHING);
                case "cooking": return SkillAPI.getBoostedLevel(Skill.COOKING);
                case "firemaking": return SkillAPI.getBoostedLevel(Skill.FIREMAKING);
                case "woodcutting": return SkillAPI.getBoostedLevel(Skill.WOODCUTTING);
                case "runecraft": case "runecrafting": return SkillAPI.getBoostedLevel(Skill.RUNECRAFT);
                case "construction": return SkillAPI.getBoostedLevel(Skill.CONSTRUCTION);
                case "farming": return SkillAPI.getBoostedLevel(Skill.FARMING);
                default: return -1;
            }
        }
        catch (Exception e)
    ...
    {
        if (itemPattern == null || itemPattern.isEmpty()) return false;
    ...
    {
        if (itemPattern == null || itemPattern.isEmpty()) return 0;
    ...
    private boolean isTargetDiagonal()
    {
        ...
        try
        {
            ...
            if (playerPos == null || targetPos == null) return false;
    ...
    {
        if (currentTarget == null || prayerType == null) return false;
        
        try
        {
            // Check overhead icon
            HeadIcon headIcon = currentTarget.getOverheadIcon();
            if (headIcon == null) return false;
            
            String lower = prayerType.toLowerCase();
            switch (lower)
            {
                case "magic": case "mage":
                    return headIcon == HeadIcon.MAGIC;
                case "ranged": case "range": case "missiles":
    ...
    {
        if (client == null || client.getLocalPlayer() == null) return 0;
    ...
    {
        if (memKey == null || memKey.isEmpty()) return false;
        ...
        return !value.isEmpty() && !value.equalsIgnoreCase("false") && !value.equals("0") && !value.equalsIgnoreCase("off");
    }
