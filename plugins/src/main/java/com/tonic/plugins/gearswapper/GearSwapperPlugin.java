...
import com.tonic.plugins.gearswapper.humanized.HumanizedActionQueue;
import com.tonic.plugins.gearswapper.humanized.HumanizedMouseHelper;
...
public class GearSwapperPlugin extends Plugin {
    ...
    public void setCurrentTarget(Player currentTarget) {
        ...
        if (currentTarget != null) {
            ...
            // Also set cached target so commands like Move can use it immediately
            this.cachedTarget = currentTarget;
            this.cachedTargetTime = System.currentTimeMillis();
            Logger.info("[Gear Swapper] Target set to: " + currentTargetName);
        } else {
    ...
    // WaitAnim state - waits for specific animation before continuing script
    private volatile int waitAnimId = -1;
    private volatile long waitAnimStartTime = 0;
    private volatile String waitAnimRemainingScript = null;
    private volatile boolean waitAnimAttackFlag = false;
    private volatile String currentExecuteScript = null; // Track script for WaitAnim remaining extraction
    private static final long WAIT_ANIM_TIMEOUT_MS = 3000; // 3 second timeout
    ...
    private HumanizedActionQueue humanizedQueue;
    private TickTimePredictor tickPredictor;
    ...
    /**
     * Toggle a prayer by name - clicks the prayer regardless of current state.
     * Unlike activatePrayerByName which only turns ON, this toggles ON/OFF.
     */
    public void togglePrayerByName(String prayerName) {
        if (prayerName == null || prayerName.isEmpty()) {
            Logger.warn("[Gear Swapper] togglePrayerByName called with empty name");
            return;
        }
        ...
        try {
            Static.invoke(() -> {
                ...
                if (prayer == null) {
                    Logger.warn("[Gear Swapper] Unknown prayer: " + prayerName);
                    return;
                }

                // Use toggle() to click regardless of current state
                if (prayer.isActive()) {
                    prayer.turnOff();
                    Logger.norm("[Gear Swapper] Toggled OFF prayer: " + prayerName);
                } else {
                    prayer.turnOn();
                    Logger.norm("[Gear Swapper] Toggled ON prayer: " + prayerName);
                }
            });
        } catch (Throwable e) {
            Logger.error("[Gear Swapper] Error toggling prayer: " + e.getMessage());
        }
    }
    ...
    protected void startUp() throws Exception {
        ...
        try {
            ...
            Logger.norm("[NoidSwap] v1.6.77 - Config with sections!");
            ...
            if (config.inventoryUtilsEnabled()) {
                ...
                inventoryUtilsOverlay.startup(); // Re-register mouse listener
            }

            // Start the trigger engine with error handling
            if (triggerEngine != null) {
                eventBus.register(triggerEngine);
                triggerEngine.start();
            ...
            // Initialize humanized mouse components
            humanizedQueue = new HumanizedActionQueue(client);
            tickPredictor = new TickTimePredictor();
    ...
    public void onAnimationChanged(AnimationChanged event) {
        try {
            ...
            // Check if we're waiting for a specific animation (WaitAnim command)
            if (waitAnimId != -1 && client != null && client.getLocalPlayer() != null) {
                Actor actor = event.getActor();
                if (actor == client.getLocalPlayer() && actor.getAnimation() == waitAnimId) {
                    Logger.norm("[Gear Swapper] WaitAnim: Detected animation " + waitAnimId + " - resuming script");
                    resumeWaitAnim();
                    return;
                }
            }
    ...
    private void executeCommandsInternal(String executeStr, boolean[] shouldAttack) {
        ...
        // Store script for WaitAnim command to extract remaining lines
        currentExecuteScript = executeStr;
    ...
    private void executeSingleCommand(String trimmed, boolean[] shouldAttack) {
        ...
        if (trimmed.contains(":")) {
            ...
            if (parts.length >= 2) {
                ...
                } else if (command.equalsIgnoreCase("WaitAnim")) {
                    // WaitAnim:animId - Wait up to 3 seconds for specific animation
                    try {
                        int animId = Integer.parseInt(value.trim());
                        // Store remaining script (commands after this one)
                        String[] allLines = currentExecuteScript != null ? currentExecuteScript.split("\n")
                                : new String[0];
                        StringBuilder remaining = new StringBuilder();
                        boolean foundWaitAnim = false;
                        for (String scriptLine : allLines) {
                            if (foundWaitAnim) {
                                if (remaining.length() > 0)
                                    remaining.append("\n");
                                remaining.append(scriptLine);
                            }
                            if (scriptLine.trim().toLowerCase().startsWith("waitanim:")) {
                                foundWaitAnim = true;
                            }
                        }
                        ...
                        // Set wait state
                        waitAnimId = animId;
                        waitAnimStartTime = System.currentTimeMillis();
                        waitAnimRemainingScript = remaining.toString();
                        waitAnimAttackFlag = shouldAttack[0];

                        Logger.norm("[Gear Swapper] WaitAnim: Waiting for animation " + animId + " (max 3s)");
                        throw new ScriptStopException("WaitAnim:" + animId);
                    } catch (NumberFormatException e) {
                        Logger.warn("[Gear Swapper] Invalid WaitAnim format: " + value + " (expected WaitAnim:animId)");
                    }
                }
            }
        }
        ...
        } else if (trimmed.equalsIgnoreCase("FindTarget")) {
            // FindTarget: Find closest attackable player and set as target
            // Only executes if we don't have a target yet
            executeFindTargetCommand();
        } else if (trimmed.equalsIgnoreCase("SetHoveredTarget") ||
                trimmed.equalsIgnoreCase("SetHoverdTarget") ||
                trimmed.equalsIgnoreCase("HoverTarget")) {
            // SetHoveredTarget: Set the player under mouse cursor as target
            executeSetHoveredTargetCommand();
        } else {
        ...
        try {
            Runnable equipTask = () -> {
                ...
                for (ItemEx item : InventoryAPI.getItems()) {
                    ...
                    boolean compatibleActionFound = false;
                    for (String actionName : new String[] { "Wear", "Equip", "Wield" }) {
                        for (String action : actions) {
                            if (action != null && action.equalsIgnoreCase(actionName)) {
                                ...
                                compatibleActionFound = true;
                                break;
                            }
                        }
                        if (compatibleActionFound)
                            break;
                    }
                    ...
                    // If we found an item, handle it
                    if (config.enableHumanizedMouse() && humanizedQueue != null) {
                        // Queue humanized action
                ...
                // If we get here and foundItem is null (and not multiMatch loop), we failed to
                // find
                if (foundItem == null && !multiMatch) {
        ...
        try {
            ...
            Runnable castTask = () -> {
                ...
                if (config.enableHumanizedMouse() && humanizedQueue != null) {
                    Ancient spell = getAncientSpell(lower);
                    if (spell != null) {
                        Rectangle bounds = HumanizedMouseHelper.getWidgetBounds(spell.getWidget());
                        if (bounds != null) {
                            Point target = HumanizedMouseHelper.getRandomPointInBounds(bounds);
                            ...
                            humanizedQueue.queue(null, () -> {
                                ...
                                if (targetP != null) {
                                    ...
                                    Logger.norm("[Humanized] Cast on target: " + targetP.getName());
                                }
        ...
        try {
            ...
            Static.invoke(() -> {
                ...
                for (String prayerName : prayerOptions) {
                    ...
                    if (config.enableHumanizedMouse() && humanizedQueue != null) {
                        Rectangle bounds = HumanizedMouseHelper.getWidgetBounds(prayer.getInterfaceId());
                        if (bounds != null) {
                            Point target = HumanizedMouseHelper.getRandomPointInBounds(bounds);
                            final PrayerAPI prayerFinal = prayer;
                            humanizedQueue.queue(target, () -> {
                                if (!prayerFinal.isActive()) {
                                    HumanizedMouseHelper.dispatchClick(client.getCanvas(), target.x, target.y);
                                    Logger.norm("[Humanized] Clicked prayer: " + prayerName);
                                }
                            }, "Px " + prayerName);
                            prayerClickCooldowns.put(prayer, client != null ? client.getTickCount() : 0);
                            continue;
                        }
                    }
                    ...
                    // Note: prayer activation is async via game server, so isActive() check
                    // immediately after turnOn() is unreliable. Just log the action taken.
                    if (active) {
                        Logger.norm("[Gear Swapper] Enabled prayer: " + prayerName);
                    } else {
                        // Don't log "failed" since it might just be async - it probably worked
                        Logger.norm("[Gear Swapper] Activating prayer: " + prayerName);
                    }
                }
            });
        } catch (Throwable e) {
        ...
        try {
            Static.invoke(() -> {
                ...
                try {
                    ...
                    if (config.enableHumanizedMouse() && humanizedQueue != null) {
                        Rectangle bounds = HumanizedMouseHelper
                                .getWidgetBounds(InterfaceID.CombatInterface.SPECIAL_ATTACK);
                        if (bounds != null) {
                            Point target = HumanizedMouseHelper.getRandomPointInBounds(bounds);
                            humanizedQueue.queue(target, () -> {
                                HumanizedMouseHelper.dispatchClick(client.getCanvas(), target.x, target.y);
                                Logger.norm("[Humanized] Clicked Special Attack");
                            }, "Special Attack");
                        } else {
    ...
    public void onGameTick(GameTick event) {
        ...
        // Check WaitAnim timeout
        checkWaitAnimTimeout();
    ...
    private void executeLooperCommand(String command, boolean[] shouldAttack) {
        ...
        if (lower.equals("findtarget")) {
            executeFindTargetCommand();
            return;
        }
        if (lower.equals("sethoveredtarget")) {
            executeSetHoveredTargetCommand();
            return;
        }
    ...
    /**
     * Resume script execution after WaitAnim detected the expected animation.
     */
    private void resumeWaitAnim() {
        if (waitAnimRemainingScript != null && !waitAnimRemainingScript.isEmpty()) {
            String remaining = waitAnimRemainingScript;
            boolean attackFlag = waitAnimAttackFlag;

            // Clear wait state first
            cancelWaitAnim();

            // Execute remaining commands
            Logger.norm("[Gear Swapper] WaitAnim: Executing remaining script");
            executeCommands(remaining, attackFlag);
        } else {
            cancelWaitAnim();
        }
    }
    ...
    /**
     * Cancel any active WaitAnim wait (called when new loadout starts or timeout).
     */
    private void cancelWaitAnim() {
        waitAnimId = -1;
        waitAnimStartTime = 0;
        waitAnimRemainingScript = null;
        waitAnimAttackFlag = false;
    }

    /**
     * Check if WaitAnim has timed out (called from onGameTick).
     */
    private void checkWaitAnimTimeout() {
        if (waitAnimId != -1) {
            long elapsed = System.currentTimeMillis() - waitAnimStartTime;
            if (elapsed > WAIT_ANIM_TIMEOUT_MS) {
                Logger.norm("[Gear Swapper] WaitAnim: Timeout waiting for animation " + waitAnimId + " - canceling");
                cancelWaitAnim();
            }
        }
    }
    ...
    /**
     * Find closest attackable player and set as target.
     * Respects wilderness levels if in wilderness.
     */
    private void executeFindTargetCommand() {
        if (client == null)
            return;

        // ONLY IF WE DONT HAVE TARGET YET!
        if (currentTarget != null) {
            return;
        }
        ...
        int wildyLevel = com.tonic.api.game.GameAPI.getWildyLevel();
        ...
        for (Player p : client.getPlayers()) {
            ...
            // Basic checks for valid target (alive)
            if (p.getHealthRatio() == 0)
                continue;

            // Calculate distance
            int dist = local.getWorldLocation().distanceTo(p.getWorldLocation());

            // Check wilderness range if in wilderness
            if (wildyLevel > 0) {
                int targetCombat = p.getCombatLevel();
        ...
        if (closest != null) {
            ...
            currentTargetName = closest.getName();

            // IMPORTANT: Set cachedTarget so commands like Cast:TB use it immediately
            cachedTarget = closest;
            cachedTargetTime = System.currentTimeMillis();

            Logger.norm("[Gear Swapper] FindTarget: Found " + currentTargetName + " (Lvl " + closest.getCombatLevel()
                    + ") at dist " + closestDist);
        } else {
            // Logger.norm("[Gear Swapper] FindTarget: No valid target found");
        }
    }

    /**
     * Sets the player currently under the mouse cursor as the target.
     * Called by SetHoveredTarget command.
     * Uses menu entries which is more reliable than convex hull detection.
     */
    private void executeSetHoveredTargetCommand() {
        if (client == null)
            return;
        ...
        String hoveredPlayerName = null;

        // Method 1: Check menu entries - most reliable since game determines what's
        // hovered
        try {
            net.runelite.api.MenuEntry[] menuEntries = client.getMenu().getMenuEntries();
            if (menuEntries != null) {
                for (net.runelite.api.MenuEntry entry : menuEntries) {
                    if (entry == null)
                        continue;

                    // Check if this menu entry is for a player (Attack option on player)
                    String target = entry.getTarget();
                    net.runelite.api.MenuAction type = entry.getType();

                    // Player interactions have specific menu action types
                    if (type == net.runelite.api.MenuAction.PLAYER_FIRST_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_SECOND_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_THIRD_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_FOURTH_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_FIFTH_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_SIXTH_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_SEVENTH_OPTION ||
                            type == net.runelite.api.MenuAction.PLAYER_EIGHTH_OPTION) {

                        // Extract player name from target (format is usually
                        // "<col=ffffff>PlayerName<col=ff00> (level-123)")
                        if (target != null && !target.isEmpty()) {
                            // Strip color tags and extract name
                            String cleanTarget = target.replaceAll("<[^>]+>", "").trim();
                            // Remove level info in parentheses
                            int parenIdx = cleanTarget.indexOf("(");
                            if (parenIdx > 0) {
                                hoveredPlayerName = cleanTarget.substring(0, parenIdx).trim();
                            } else {
                                hoveredPlayerName = cleanTarget.trim();
                            }
                            break;
                        }
                    }
            ...
            Logger.norm("[Gear Swapper] SetHoveredTarget: Menu check error: " + e.getMessage());
        }

        // Find the actual Player object by name
        if (hoveredPlayerName != null && !hoveredPlayerName.isEmpty()) {
            for (Player p : client.getPlayers()) {
                if (p == null || p == local || p.getName() == null)
                    continue;
                if (p.getName().equalsIgnoreCase(hoveredPlayerName)) {
                    hoveredPlayer = p;
                    break;
                }
            }
        ...
        if (hoveredPlayer != null) {
            setCurrentTarget(hoveredPlayer);
            Logger.norm("[Gear Swapper] SetHoveredTarget: Targeted " + hoveredPlayer.getName()
                    + " (Lvl " + hoveredPlayer.getCombatLevel() + ")");
        } else {
            Logger.norm("[Gear Swapper] SetHoveredTarget: No player hovered (hover over a player first)");
        }
    }
}
