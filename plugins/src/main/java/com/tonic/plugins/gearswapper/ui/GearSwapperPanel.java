...
public class GearSwapperPanel extends PluginPanel
{
    ...
    public GearSwapperPanel()
    {
        ...
        System.out.println("[Gear Swapper DEBUG] About to build click heatmap section...");
        JPanel heatmapSection = buildClickHeatmapSection();
        content.add(heatmapSection);
        System.out.println("[Gear Swapper DEBUG] Click heatmap section added successfully");
        
        content.add(Box.createVerticalStrut(8));
        content.add(buildGearButton());
        System.out.println("[Gear Swapper DEBUG] Gear button added");
    ...
    {
        System.out.println("[Gear Swapper DEBUG] setPlugin called with plugin: " + (plugin != null ? "NOT NULL" : "NULL"));
        this.plugin = plugin;
    ...
    private void rebuildPanelContent()
    {
        ...
        System.out.println("[Gear Swapper DEBUG] About to build click heatmap section (rebuild)...");
        JPanel heatmapSection = buildClickHeatmapSection();
        content.add(heatmapSection);
        System.out.println("[Gear Swapper DEBUG] Click heatmap section added successfully (rebuild)");

        content.add(Box.createVerticalStrut(8));
        content.add(buildGearButton());
        System.out.println("[Gear Swapper DEBUG] Gear button added (rebuild)");
        content.add(Box.createVerticalStrut(12));
        content.add(buildHeader());
        content.add(Box.createVerticalStrut(12));
    ...
    private JPanel buildClickHeatmapSection()
    {
        ...
        try {
            ...
            // Force 1 tick threshold slider (initially hidden until Force 1 tick is enabled)
    ...
    private JPanel buildHeader()
    {
        ...
        final JLabel title = new JLabel("⚔️ Gear Swapper v2.1");
    ...
    {
        if (loadoutsContentPanel == null) return;
    ...
    private JPanel createLoadoutCard(int loadoutNum)
    {
        ...
        nameField.getDocument().addDocumentListener(new DocumentListener() {
            @Override public void insertUpdate(DocumentEvent e) { data.name = nameField.getText(); saveLoadoutToConfig(loadoutNum); }
            @Override public void removeUpdate(DocumentEvent e) { data.name = nameField.getText(); saveLoadoutToConfig(loadoutNum); }
            @Override public void changedUpdate(DocumentEvent e) { data.name = nameField.getText(); saveLoadoutToConfig(loadoutNum); }
        });
        ...
        commandsArea.getDocument().addDocumentListener(new DocumentListener() {
            @Override public void insertUpdate(DocumentEvent e) { data.items = commandsArea.getText(); saveLoadoutToConfig(loadoutNum); updateTextAreaHeight(commandsArea); }
            @Override public void removeUpdate(DocumentEvent e) { data.items = commandsArea.getText(); saveLoadoutToConfig(loadoutNum); updateTextAreaHeight(commandsArea); }
            @Override public void changedUpdate(DocumentEvent e) { data.items = commandsArea.getText(); saveLoadoutToConfig(loadoutNum); updateTextAreaHeight(commandsArea); }
        });
    ...
    private void openScriptEditor(int loadoutNum, LoadoutData data, JTextArea inlineCommandsArea)
    {
        ...
        GhostTextArea editorArea = new GhostTextArea(data.items);
        editorArea.setFont(new Font("Consolas", Font.PLAIN, 11));
        editorArea.setLineWrap(true);
        editorArea.setWrapStyleWord(true);
        editorArea.setFocusTraversalKeysEnabled(false); // Keep Tab inside the editor
        ...
        JLabel hint = new JLabel("Use multiple lines and conditions (if, &&, ||, >, <, etc.) just like in the inline box.");
        hint.setFont(new Font("Whitney", Font.PLAIN, 10));
        hint.setForeground(new Color(160, 170, 185));
        panel.add(hint, BorderLayout.SOUTH);

        // Keep data + inline area in sync while typing
        editorArea.getDocument().addDocumentListener(new DocumentListener()
        {
            ...
            private void sync()
            {
                ...
                {
                    inlineCommandsArea.setText(text);
        ...
        // Attach autocomplete (Ctrl+Space) for commands/conditions/items
    ...
    private void openLooperEditor(JTextArea inlineArea)
    {
        ...
        // Attach autocomplete for commands/conditions/items
        attachAutocomplete(editorArea);
    ...
    /**
     * Validate an if-block that may span multiple lines, including a possible else-block.
    ...
    private int validateIfBlock(String[] lines, int startIndex, java.util.List<String> messages)
    {
        ...
        // For now we don't deeply validate the inner commands or else-block structure here;
        // we just ensure braces exist somewhere after the if. Returning closeLine causes
        ...
        if (trimmed.contains(":"))
        {
            ...
            if (parts.length >= 2)
            {
                ...
                if (!("item".equals(cmd) || "cast".equals(cmd) || "prayer".equals(cmd) ||
                      "special".equals(cmd) || "move".equals(cmd)))
                {
                    messages.add("Line " + lineNo + ": WARNING - unknown command prefix '" + parts[0].trim() + "'.");
                }
            }
        }
    }
    ...
    private void validateConditionSyntax(String condition, int lineNo, java.util.List<String> messages)
    {
        ...
        // Allowed variables that the engine understands
        java.util.Set<String> allowedVars = new java.util.HashSet<>();
        allowedVars.add("frozen");
        allowedVars.add("target_frozen");
        allowedVars.add("target frozen");
        allowedVars.add("self_frozen");
        allowedVars.add("player_frozen");
        allowedVars.add("me_frozen");
        allowedVars.add("self frozen");
        allowedVars.add("player frozen");
        allowedVars.add("me frozen");
        allowedVars.add("has_target");
        allowedVars.add("target_exists");
        allowedVars.add("has target");
        allowedVars.add("has_cached_target");
        allowedVars.add("cached_target");
        allowedVars.add("spec");
        allowedVars.add("spec_energy");
        allowedVars.add("special");
        allowedVars.add("special_energy");
        allowedVars.add("hp");
        allowedVars.add("health");
        allowedVars.add("hitpoints");
        allowedVars.add("prayer");
        allowedVars.add("pray");
        allowedVars.add("distance");
        allowedVars.add("target_distance");
        allowedVars.add("player_frozen_ticks");
        allowedVars.add("self_frozen_ticks");
        allowedVars.add("target_frozen_ticks");
        allowedVars.add("ticks_since_swap");
        allowedVars.add("swap_ticks");
    ...
    private void showAutocompleteFromButton(JTextArea editorArea)
    {
        ...
        java.util.List<String> baseSuggestions = (java.util.List<String>) editorArea.getClientProperty("gs-autocomplete-base");
    ...
    private java.util.List<String> buildAutocompleteBaseList()
    {
        ...
        // Condition helpers / common patterns
        list.add("frozen");
        list.add("self_frozen");
        list.add("has_target");
        list.add("has_cached_target");
        list.add("spec>=50");
        list.add("hp<40");
        list.add("distance<=2");
        list.add("ticks_since_swap>=6");

        // Raw condition variables (so they can be completed too)
        list.add("spec");
        list.add("spec_energy");
        list.add("special_energy");
        list.add("hp");
        list.add("health");
        list.add("hitpoints");
        list.add("prayer");
        list.add("distance");
        list.add("target_distance");
    ...
    private void cancelKeybindCapture()
    {
        if (capturingButton != null && capturingLoadoutNum > 0) {
            LoadoutData data = loadoutData.get(capturingLoadoutNum);
            String displayText = data != null && data.keybind != null ? 
                "🔗 " + data.keybind.toString() : "🔗 Set Keybind";
    ...
    public void saveLoadoutToConfig(int loadoutNum)
    {
        ...
        if (data != null) {
            ...
            if (data.keybind != null) {
                configManager.setConfiguration(GearSwapperConfig.GROUP, "loadout" + loadoutNum + "Keybind", data.keybind.toString());
            }
        }
    }
    ...
    private void savePresetToConfig(String presetName)
    {
        ...
        if (preset != null) {
            ...
            for (Map.Entry<Integer, LoadoutData> entry : preset.loadouts.entrySet()) {
                ...
                if (data.keybind != null) {
                    configManager.setConfiguration(GearSwapperConfig.GROUP, loadoutKey + "_keybind", data.keybind.toString());
                }
            }
        }
    }
    ...
    private void deletePresetFromConfig(String presetName)
    {
        ...
        // Remove preset data
        // Note: ConfigManager doesn't provide a way to remove all keys matching a pattern
        // For now we'll just leave the old data, it will be overwritten if the preset is recreated
    }
    ...
    private void loadSavedLoadouts()
    {
        ...
        if (configManager != null) {
            ...
            for (int i = 1; i <= 50; i++) {
                ...
                Boolean attack = configManager.getConfiguration(GearSwapperConfig.GROUP, "loadout" + i + "Attack", Boolean.class);
                ...
                if (name != null && !name.trim().isEmpty()) {
                    ...
                    System.out.println("[Gear Swapper UI] Loaded loadout " + i + ": " + name + " (attack=" + data.attack + ")");
                }
            }
            ...
            switch (part.trim()) {
                ...
                default:
                    ...
                    if (part.length() == 1) {
                        ...
                        // Handle special keys
                        switch (part) {
                            case "enter": keyCode = KeyEvent.VK_ENTER; break;
                            case "space": keyCode = KeyEvent.VK_SPACE; break;
                            case "tab": keyCode = KeyEvent.VK_TAB; break;
                            case "escape": keyCode = KeyEvent.VK_ESCAPE; break;
                            case "delete": keyCode = KeyEvent.VK_DELETE; break;
                            case "backspace": keyCode = KeyEvent.VK_BACK_SPACE; break;
            ...
            if (presetNames != null && !presetNames.trim().isEmpty()) {
                ...
                for (String name : names) {
                    if (!name.trim().isEmpty()) {
                        ...
                        for (int i = 1; i <= 50; i++) {
                            String loadoutName = configManager.getConfiguration(GearSwapperConfig.GROUP, prefix + i + "_name");
                            String loadoutItems = configManager.getConfiguration(GearSwapperConfig.GROUP, prefix + i + "_items");
                            String loadoutKeybind = configManager.getConfiguration(GearSwapperConfig.GROUP, prefix + i + "_keybind");
...
class SimpleGearManagerPanel extends JPanel
{
    ...
    public SimpleGearManagerPanel(Map<String, GearSwapperPanel.GearSet> gearSets, ConfigManager configManager)
    {
        ...
        System.out.println("[Gear Swapper DEBUG] Received gear sets: " + (gearSets != null ? gearSets.size() : "null"));
        System.out.println("[Gear Swapper DEBUG] Received config manager: " + (configManager != null ? "present" : "null"));
    ...
    private class ItemSearchPanel extends JPanel
    {
            ...
            new Thread(() -> {
                try {
                    ...
                    for (String searchTerm : searchTerms) {
                        try {
                            String searchUrl = apiUrl + "&search=" + java.net.URLEncoder.encode(searchTerm, "UTF-8");
                            String jsonText = readUrl(searchUrl);
                            List<String> searchResults = parseOpenSearchResults(jsonText);
                            
                            System.out.println("[Gear Swapper DEBUG] Search '" + searchTerm + "' found " + searchResults.size() + " items");
                            ...
                            System.out.println("[Gear Swapper DEBUG] Search for '" + searchTerm + "' failed: " + e.getMessage());
                    ...
                    System.out.println("[Gear Swapper DEBUG] Loaded " + allItems.size() + " unique equipment items from API");
        ...
        private List<String> parseOpenSearchResults(String json) {
            ...
            try {
                // OpenSearch returns JSON array with [search terms, item titles, descriptions, urls]
                    ...
                    if (secondBracket > 0 && endBracket > secondBracket) {
                        ...
                        while (matcher.find()) {
                            ...
                            if (!title.contains(":") && !title.contains("(") && !title.contains("Category") && 
                                !title.contains("File") && !title.contains("Help") && !title.contains("Template") &&
        ...
        private void searchItemsFromAPI(String query)
        {
            ...
            new Thread(() -> {
                try {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        System.out.println("[Gear Swapper DEBUG] Local search '" + query + "' found " + localResults.size() + " results");
                    ...
                    try {
                        ...
                        SwingUtilities.invokeLater(() -> {
                            System.out.println("[Gear Swapper DEBUG] API search '" + query + "' found " + filteredApiResults.size() + " additional results");
                            System.out.println("[Gear Swapper DEBUG] Combined search '" + query + "' found " + combinedResults.size() + " total results");
                            displayItems(combinedResults.toArray(new String[0]));
                        });
                        
                    } catch (Exception e) {
                        System.out.println("[Gear Swapper DEBUG] API search failed for '" + query + "': " + e.getMessage());
        ...
        private void showQuickMatches(String query)
        {
            ...
            for (String item : allItems) {
                ...
                if (lowerItem.startsWith(query)) {
                    ...
                    if (quickMatches.size() >= 20) break; // Limit to 20 for quick display
                }
            }
        ...
        private void loadPopularItems()
        {
            if (!apiLoaded) {
                ...
                String[] popularItems = {
                    "Dragon claws", "Abyssal whip", "Bandos godsword", "Armadyl godsword", "Saradomin godsword", "Zamorak godsword",
                    "Dragon scimitar", "Dragon dagger", "Dragon mace", "Dragon longsword", "Dragon battleaxe", "Dragon halberd",
                    "Abyssal bludgeon", "Saradomin sword", "Staff of the dead", "Trident of the seas", "Trident of the swamp",
                    "Toxic blowpipe", "Magic shortbow", "Twisted bow", "Dragon hunter crossbow", "Armadyl crossbow",
                    "Elder maul", "Granite maul", "Ghrazi rapier", "Scythe of vitur", "Inquisitor's mace",
                    "Bandos chestplate", "Bandos tassets", "Bandos boots", "Torva full helm", "Torva platebody", "Torva platelegs",
                    "Pernix cowl", "Pernix body", "Pernix chaps", "Virtus mask", "Virtus robe top", "Virtus robe legs",
                    "Dragon full helm", "Dragon chainbody", "Dragon platelegs", "Dragon boots", "Dragon sq shield",
                    "Granite shield", "Blessed spirit shield", "Elysian spirit shield", "Arcane spirit shield", "Spectral spirit shield",
                    "Justiciar faceguard", "Justiciar chestguard", "Justiciar legguards", "Ancestral robe top", "Ancestral robe bottom",
                    "Armadyl helmet", "Armadyl chestplate", "Armadyl chainskirt", "Karil's coif", "Karil's leathertop", "Karil's leatherskirt",
                    "Black d'hide body", "Black d'hide chaps", "Black d'hide vambraces", "Guthix chaps", "Zamorak chaps", "Armadyl chaps",
                    "Dragonhide body", "Dragonhide chaps", "Dragonhide vambraces", "Blessed dragonhide body", "Blessed dragonhide chaps",
                    "Masori mask", "Masori body", "Masori chaps", "Fremennik helm", "Fremennik shirt", "Fremennik kilt",
                    "Ancestral hat", "Ancestral robe top", "Ancestral robe bottom", "Kodai wand", "Master wand", "Ancient staff",
                    "Slayer's staff", "Staff of light", "Imbued god cape", "Imbued saradomin cape", "Imbued guthix cape", "Imbued zamorak cape",
                    "3rd age mage hat", "3rd age robe top", "3rd age robe bottom", "Mystic hat", "Mystic robe top", "Mystic robe bottom",
                    "Infinity hat", "Infinity top", "Infinity bottoms", "Splitbark helm", "Splitbark body", "Splitbark gauntlets", "Splitbark boots",
                    "Amulet of fury", "Amulet of glory", "Amulet of torture", "Amulet of rancor", "Amulet of the damned", "Amulet of avasice",
                    "Amulet of strength", "Amulet of magic", "Amulet of power", "Amulet of accuracy", "Amulet of defence",
                    "Occult necklace", "Necklace of anguish", "Berserker necklace", "Salve amulet", "Salve amulet(ei)",
                    "3rd age amulet", "Amulet of eternal glory", "Amulet of bounty", "Amulet of zealots",
                    "Fire cape", "Infernal cape", "Ardyne cloak", "Max cape", "Cape of accomplishment", "Team cape",
                    ...
                    "Berserker ring", "Warrior ring", "Archers ring", "Seers ring", "Tyrannical ring", "Treasonous ring",
                    "Ring of the gods", "Ring of suffering", "Ring of endurance", "Ring of vigour", "Ring of shadows",
                    "3rd age ring", "Dragonstone ring", "Emerald ring", "Ruby ring", "Diamond ring", "Onyx ring",
                    "Barrows gloves", "Dragon gloves", "Rune gloves", "Adamant gloves", "Mithril gloves", "Black gloves",
                    "Steel gloves", "Iron gloves", "Bronze gloves", "Leather gloves", "Gloves of silence", "Recipe for disaster",
                    "Regen bracelet", "Combat bracelet", "Amulet of eternal glory", "Explorer's ring", "Karamja gloves",
                    "Dragon boots", "Bandos boots", "Primordial boots", "Pegasian boots", "Eternal boots", "Infinity boots",
                    "Rune boots", "Adamant boots", "Mithril boots", "Black boots", "Steel boots", "Iron boots", "Bronze boots",
                    "Climbing boots", "Frog leather boots", "Spiked manacles", "Boots of brimstone", "Boots of stone",
                    "Dragon defender", "Avernic defender", "Rune defender", "Adamant defender", "Mithril defender", "Black defender",
                    "Steel defender", "Iron defender", "Bronze defender", "Toktz-ket-xil", "Dragon kiteshield", "Rune kiteshield",
                    "Adamant kiteshield", "Mithril kiteshield", "Black kiteshield", "Steel kiteshield", "Iron kiteshield",
                    "Spirit shield", "Blessed spirit shield", "Arcane spirit shield", "Spectral spirit shield", "Elysian spirit shield",
                    "Dragon arrows", "Rune arrows", "Adamant arrows", "Mithril arrows", "Broad arrows", "Amethyst arrows",
                    "Dragon darts", "Rune darts", "Adamant darts", "Mithril darts", "Broad darts", "Amethyst darts",
                    "Dragon bolts", "Rune bolts", "Adamant bolts", "Mithril bolts", "Broad bolts", "Diamond bolts",
                    ...
                    "Barrows gloves", "Fighter torso", "Dragonfire shield", "Anti-dragon shield", "Ancient shield",
                    "Void knight top", "Void knight robe", "Void knight gloves", "Void mage helm", "Void ranger helm",
                    "Elite void top", "Elite void robe", "Void knight seal", "Void knight mace", "Void knight maul",
                    ...
                    "Air rune", "Water rune", "Earth rune", "Fire rune", "Mind rune", "Body rune", "Cosmic rune",
                    "Chaos rune", "Nature rune", "Law rune", "Death rune", "Blood rune", "Soul rune", "Astral rune",
