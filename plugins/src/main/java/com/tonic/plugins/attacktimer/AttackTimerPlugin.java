...
public class AttackTimerPlugin extends Plugin
{
    ...
    protected void startUp() throws Exception
    {
        ...
        updatePanel();
        Logger.norm("[AttackTimer] AttackTimer v2.3 - startup complete");
        ...
        safeRegister(duelChallengeHandler);
        Logger.norm("[AttackTimer] DuelChallengeHandler registered (raw bus)");
        
        // Initialize PID detector
        pidDetector = new PidDetector(config, client);
        safeRegister(pidDetector);
        
        // Initialize target spec tracker
        targetSpec = new TargetSpec(config, client);
        safeRegister(targetSpec);
    ...
    private void safeRegister(Object listener)
    {
        try
        {
            if (listener == null) return;
            ...
            try
            {
                ReflectUtil.getMethod(rawBus, "register", new Class[]{Object.class}, new Object[]{listener});
            }
            catch (Exception e1)
            {
                try
                {
                    ReflectUtil.getMethod(rawBus, "register", new Class[]{listener.getClass()}, new Object[]{listener});
                }
                catch (Exception e2)
                {
                    // Swallow to avoid noisy logs on register
                }
            }
        }
        catch (Exception ignored)
