...
public class TargetManagement {
    ...
    // ActorEx-based aggressive targeting (locked until death / override on
    // attacker)
    ...
    // Target acquisition lock - prevents re-acquiring targets during cooldown
    // Used after "already under attack" message to only accept actual attackers
    private static boolean targetAcquisitionLocked = false;
    private static long targetAcquisitionLockEndTime = 0;
    private static final long TARGET_LOCK_DURATION_MS = 2000; // 2 seconds lock
    ...
    /**
     * Lock target acquisition - only accept attackers, block our interactions.
     * Used after "already under attack" message.
     */
    public static void lockTargetAcquisition() {
        targetAcquisitionLocked = true;
        targetAcquisitionLockEndTime = System.currentTimeMillis() + TARGET_LOCK_DURATION_MS;
        Logger.norm("[TargetManagement] Target acquisition LOCKED for " + TARGET_LOCK_DURATION_MS
                + "ms - only accepting attackers");
    }

    /**
     * Check if target acquisition is locked (will auto-expire after timeout)
     */
    public static boolean isTargetAcquisitionLocked() {
        if (!targetAcquisitionLocked) {
            return false;
        }
        // Auto-expire lock
        if (System.currentTimeMillis() > targetAcquisitionLockEndTime) {
            targetAcquisitionLocked = false;
            Logger.norm("[TargetManagement] Target acquisition lock EXPIRED");
            return false;
        }
        return true;
    }
    ...
    /**
     * Clear target acquisition lock (called when attacker is found)
     */
    public static void clearTargetAcquisitionLock() {
        if (targetAcquisitionLocked) {
            targetAcquisitionLocked = false;
            Logger.norm("[TargetManagement] Target acquisition lock CLEARED (attacker found)");
        }
    }
    ...
    public static void onInteractingChanged(InteractingChanged event) {
        ...
        if (source != null && source instanceof Player) {
            ...
            if (isLocalPlayer(player)) {
                ...
                if (target != null && target instanceof Player) {
                    ...
                    // BLOCK our interactions during target acquisition lock
                    // This prevents re-targeting the player we just tried to attack
                    // after receiving "already under attack" message
                    if (isTargetAcquisitionLocked()) {
                        Logger.norm("[TargetManagement] BLOCKED: Local interaction with " + targetPlayer.getName()
                                + " during lock");
                        return; // Don't set target from our interaction
                    }
                    ...
                    if (LmsState.isIgnored(targetPlayer)) {
                        Logger.norm("[TargetManagement] Local player interacted with ignored player: "
                                + targetPlayer.getName() + " - not setting target.");
                    } else {
                        setTarget(targetPlayer.getName(), true);
                        Logger.norm("[TargetManagement] Local player interacting with: " + targetPlayer.getName());
                    }
                } else {
                    // Local player stopped interacting - don't clear immediately, let timeout
                    // handle it
        ...
        // Check if local player is being targeted (ALWAYS works, even during lock)
        // This is what finds the actual attacker after "already under attack"
        if (target != null && target instanceof Player) {
            ...
            if (isLocalPlayer(player)) {
                ...
                if (source != null && source instanceof Player) {
                    ...
                    // During lock, this is the ONLY way to set target (attacker found!)
                    if (isTargetAcquisitionLocked()) {
                        Logger.norm("[TargetManagement] ATTACKER FOUND during lock: " + sourcePlayer.getName());
                        clearTargetAcquisitionLock(); // Clear lock now that we have attacker
                    }
    ...
    private static void setTarget(String targetName, boolean inCombat) {
        ...
        if (!newTargetName.equals(currentTargetName)) {
            Logger.norm(
                    "[TargetManagement] Target changed from '" + currentTargetName + "' to '" + newTargetName + "'");
    ...
    public static void ignoreTargetForSeconds(Player target, int seconds) {
        ...
        Logger.norm("[TargetManagement] Ignoring " + (target != null ? target.getName() : "null") + " for " + seconds
                + "s");
    }
    ...
    private static ActorEx<?> getOurAttackerEx() {
        ...
        // Scan players directly: anyone whose interacting target is us and who is not
        // idle.
    ...
    private static boolean isInCombatWithSomeoneElse(ActorEx<?> target) {
        ...
        // If the target is interacting with someone who is NOT us, they're in combat
        // with someone else.
