...
/**
 * Computes the current combat state based on FreezeManager and distance to
 * target.
 */
public class FightStateManager {
    ...
    public static CombatState getCurrentState() {
        ...
        // Primary source: LmsState.getCurrentTarget() - this is the canonical combat
        // target
        Player target = LmsState.getCurrentTarget();

        // Fallback to FreezeManager if LmsState has no target
        if (target == null) {
            target = FreezeManager.getCurrentTarget();
        }

        // If FreezeManager has no current target, fall back to TargetManagement's
        // target name.
