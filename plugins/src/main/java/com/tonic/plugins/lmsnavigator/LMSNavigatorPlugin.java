...
import com.tonic.plugins.lmsnavigator.FightLogic.tasks.InitPhaseTask;
import com.tonic.plugins.lmsnavigator.FightLogic.AdvancedCombatStrategy;

@PluginDescriptor(name = "LMS Navigator", description = "Navigate anywhere in LMS using template-based pathfinding", tags = {
        "lms", "last man standing", "navigation", "pathfinding", "walker" })
public class LMSNavigatorPlugin extends Plugin {
    private static final String VERSION = "2.70";
    ...
    protected void startUp() throws Exception {
        instance = this;
        Logger.norm("[LMS Navigator] Plugin started v2.70 - Combat & Upgrade Fixes");

        // Initialize unified state management
        LmsState.initialize(client);
        UnifiedTargetManager.initialize(client);
        LmsState.setPluginEnabled(true);

        // Initialize advanced combat strategy
        AdvancedCombatStrategy.initialize(client);

        if (client != null) {
            Static.invoke(() -> {
                client.addChatMessage(ChatMessageType.GAMEMESSAGE, "",
                        "<col=00ff00>LMS Navigator v" + VERSION + " started. Upgrade logic enabled.</col>", null);
                return null;
            });
        }
        ...
        try {
            ...
            if (overlay != null) {
                ...
                // Set client reference for UpgradeManager (gear upgrades)
                UpgradeManager.setClient(client);
            }

            // Register fight overlay separately so it can be toggled independently in the
            // overlay manager
            ...
            Logger.error("[LMS Navigator] Error during startup: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void onInteractingChanged(InteractingChanged event) {
        // Delegate to unified target manager for consistent targeting
        UnifiedTargetManager.onInteractingChanged(event);

        // Also notify TargetManagement for compatibility
    ...
    public void onGameTick(GameTick event) {
        try {
            // Update unified target manager
            UnifiedTargetManager.onGameTick(event);

            // Update advanced combat strategy
            if (LmsState.isInLmsInstance() && LmsState.isPluginEnabled()) {
                AdvancedCombatStrategy.onGameTick();
            }

            // Update instance state
            boolean currentlyInInstance = isInInstance();

            // ALWAYS sync LmsState with current instance state
            // This fixes the bug where plugin starts while already in instance
            if (LmsState.isInLmsInstance() != currentlyInInstance) {
                LmsState.setInLmsInstance(currentlyInInstance);
            }

            // Log state changes
            if (lastInstanceState == null || !lastInstanceState.equals(currentlyInInstance)) {
                lastInstanceState = currentlyInInstance;

                if (currentlyInInstance) {
                    Logger.norm("[LMS Navigator] Entered LMS instance");
                    // Reset init phase state for new game
                    InitPhaseTask.reset();
                    // Start with INIT_PHASE when entering instance
                    if (LmsState.getCurrentTask() == LmsState.LmsTask.IDLE) {
                        TaskQueue.preempt(LmsState.LmsTask.INIT_PHASE);
                    }
                } else {
                    Logger.norm("[LMS Navigator] Left LMS instance");
                    // Reset when leaving instance
                    LmsState.reset();
                }
            }

            // Sync LmsState target with TargetManagement for compatibility
            ...
            // Check for upgrade opportunities with smart navigation respect
            if (UpgradeGearTask.shouldStartUpgrade()
                    && LmsState.getCurrentTask() != LmsState.LmsTask.UPGRADE_GEAR
                    && !LmsState.isNavigating()) { // Don't interrupt navigation
                Logger.norm("[LMS Navigator] Bloody key detected! Starting gear upgrade task.");
                TaskQueue.preempt(LmsState.LmsTask.UPGRADE_GEAR);
            }

            // Drive the task queue - ONLY when in LMS instance
            if (LmsState.isInLmsInstance() && LmsState.isPluginEnabled()) {
                // CRITICAL: Update attack timers so cooldowns decrement!
                AttackTimers.onGameTick(event);

                // CRITICAL: Update freeze timers for combat state tracking!
                FreezeManager.onGameTick(event);

                // Sync freeze state to LmsState for combat handlers
                LmsState.setFreezeState(
                        FreezeManager.getPlayerFreezeTicks() > 0,
                        FreezeManager.getTargetFreezeTicks() > 0,
                        FreezeManager.getPlayerFreezeTicks(),
                        FreezeManager.getTargetFreezeTicks());

                // AI Prayer management during combat
                if (LmsState.hasTarget() || LmsState.isInCombat()) {
                    AiPrayerManager.onGameTick();
                }

                TaskQueue.tick();

                // Safe zone management with task priority awareness
                SafeZoneManagement.onGameTick();
            }

        } catch (Exception e) {
            Logger.error("[LMS Navigator] Error in game tick: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void onChatMessage(ChatMessage event) {
        ...
        if (type.equals(ChatMessageType.SPAM)) {
            if (message.contains("i'm already under attack")) {
                Logger.norm(
                        "[LMS Navigator] Received 'already under attack' - locking target acquisition for attackers only");

                // LOCK target acquisition FIRST - this blocks our interactions from re-setting
                // target
                TargetManagement.lockTargetAcquisition();

                // Now clear target safely
                TargetManagement.clearTarget();
                LmsState.setTarget(null);

                // Force immediate attacker scan
        ...
        // Process LMS game mode detection (use original message for case-sensitive
        // matching)
    ...
    public boolean navigateToTemplate(WorldPoint target) {
        ...
        try {
            exec.submit(() -> {
                try {
                    ...
                    if (isInInstance()) {
                        ...
                        MovementAPI.walkToWorldPoint(navTarget);
                        if (!waitForArrival(navTarget, 5000)) {
                            Logger.warn("[LMS Navigator] Timeout waiting for navigation to: " + navTarget);
                        }
                    }
                } catch (Exception e) {
                    Logger.error("[LMS Navigator] Error during navigation: " + e.getMessage());
                    e.printStackTrace();
                } finally {
            ...
            Logger.error("[LMS Navigator] Error submitting navigation task: " + e.getMessage());
            e.printStackTrace();
            isNavigating.set(false);
            return false;
        }
    ...
    private void handleInitPhaseChat(ChatMessage event) {
        ...
        Logger.norm("[LMS Navigator] Saw '2...' chat, attempting auto-target. type=" + event.getType() + " name="
                + event.getName() + " text='" + plain + "'");
    ...
    private void autoTargetNearestPlayerIfNone() {
        ...
        // After issuing the attack packet, TaskQueue/EngageTargetTask will drive
        // combat.
    }
    ...
    private static String extractPlayerNameFromAlreadyFighting(String message) {
        ...
        while (end > start && (Character.isWhitespace(message.charAt(end - 1)) || message.charAt(end - 1) == '.'
