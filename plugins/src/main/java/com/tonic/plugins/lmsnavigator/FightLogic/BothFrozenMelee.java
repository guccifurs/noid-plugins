...
public class BothFrozenMelee
{
	...
	/**
	 * Tick handler for the BothFrozenMelee state (both frozen, in melee range).
	 *
	 * Rules:
	 * - Only runs when CombatState is BOTH_FROZEN_MELEE.
	 * - Attack when our CD is 1 or lower.
	 * - 40% magic (blood barrage / blood blitz), 40% melee, 20% ranged.
	...
	public static void onGameTick()
	{
		...
		// Only act in the BOTH_FROZEN_MELEE combat state
		CombatState state = FightStateManager.getCurrentState();
		if (state != CombatState.BOTH_FROZEN_MELEE)
		{
